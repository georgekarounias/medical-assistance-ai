...
- **Intake.** `POST /ingestions` with `documentType` as discriminator (SessionTranscript, DoctorNote, LabReport, ImagingReport) and per-type payload validation. Transcripts arrive as inline speaker Turns; notes as inline text; lab and imaging reports as size-capped base64 PDFs. Returns 202 with an ingestion id; 409 when the same document identity is already queued or processing.
